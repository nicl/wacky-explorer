...
JsonView = {
    ...
    addCollapser: function(item) {
        ...
        collapser.innerHTML = '-';
        collapser.addEventListener('click', this.collapse, false);
        item.insertBefore(collapser, item.firstChild);
    },

    format: function(domNode) {
        // this.debug = false;
        // if (id.indexOf("#") != -1) {
        //     this.idType = "id";
        //     this.id = id.replace('#', '');
        ...
        if (is_json || is_jsonp) {
            ...
            domNode.innerHTML = outputDoc;

            var items = document.getElementsByClassName('collapsible');
            for (var i = 0; i < items.length; i++) {
                this.addCollapser(items[i].parentNode);
            }
        } else {
